...
class Utils:
    ...
    def editops(src_list, trg_list):
        ...
        while i > 0 or j > 0:
            if bp[i, j] == 1:
                ...
                ops.append(
                    ("replace", "{}->{}".format(src_list[i - 1], trg_list[j - 1]))
                )
                j -= 1
                i -= 1
            elif bp[i, j] == 3:
    ...
    def get_rate(edit_type, src_list, trg_list):
        c = 0
        for o in Utils.editops(src_list, trg_list):
            if o[0] == edit_type:
                c += 1
        return c / len(src_list)

    def get_del_list(src_list, trg_list):
        c = 0
        errs = []
        for o in Utils.editops(src_list, trg_list):
            if o[0] == "delete":
                c += 1
                errs.append(o[1].split("->")[0])
