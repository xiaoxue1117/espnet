...
dir=$1
orig=$(echo $dir | cut -f1-3 -d"/")
mkdir -p $dir
concatjson.py ${orig}/data_ph_cat.*.json > ${dir}/data.json

if [ $num_spkrs -eq 1 ]; then
  local/json2trn_ph_ignoresp.py ${dir}/data.json --num-spkrs ${num_spkrs} --refs ${dir}/ref.trn --hyps ${dir}/hyp.trn
  ...
  local/json2trn_ph.py ${dir}/data.json --num-spkrs ${num_spkrs} --refs ${ref_trns} --hyps ${hyp_trns}
