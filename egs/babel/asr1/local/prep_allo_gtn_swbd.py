        ...
        if n > 0:
            for phoneme in range(allo_mat.shape[0]):
                if allo_mat[phoneme][phone] != 0:
                    arcs.append((0, 0, phone, phoneme, 1/n))
        #else:
        #    n = allo_mat.shape[0] - 1   # dense map to all phonemes except blank
        #    for phoneme in range(1, allo_mat.shape[0]):
        #        arcs.append((0, 0, phone, phoneme, 1/n))

    allo_gtn = gtn.Graph()
    allo_gtn.add_node(True, True)
    for a in arcs:
        allo_gtn.add_arc(a[0], a[1], a[2], a[3], math.log(a[4]))
