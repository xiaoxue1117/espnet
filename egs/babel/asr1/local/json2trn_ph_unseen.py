...
def convert(jsonf, refs, hyps, num_spkrs=1):
    ...
    for ns in range(num_spkrs):
        ...
        for x in j["utts"]:
            ...
            if num_spkrs == 1:
                seq = j["utts"][x]["output"][-1]["phones"]
                seq = [x[0] for x in groupby(seq)]
                seq = [str(x) for x in filter(lambda x: x != 0, seq)]
                #seq = [i for i in j["utts"][x]["output"][1]["rec_token"].split()]
            else:
                seq = [i for i in j["utts"][x]["output"][ns][1]["rec_token"].split()]
            # In the recognition hypothesis,
            # the <eos> symbol is usually attached in the last part of the sentence
            # and it is removed below.
            
            seq = [c for c in seq if int(c) != 22]
            ...
            # reference
            if num_spkrs == 1:
                seq = j["utts"][x]["output"][1]["token"]
            else:
                seq = j["utts"][x]["output"][ns][1]["token"]
