...
class Trainer:
    ...
    If you'd like to use multiple optimizers, then inherit this class   # ok TTI !
...
class TrainerSemiSL(Trainer):
    """"
    We inherit from trainer and will override run and train_one_epoch methods
    ...
    @staticmethod   # this one we should override later to have the two models  # TODO
    def resume(
        checkpoint: Union[str, Path],
        model: torch.nn.Module,
        reporter: Reporter,
    ...
    def run(
        cls,
        model_s: AbsESPnetModel,
        model_t: AbsESPnetModel,
        optimizers: Sequence[torch.optim.Optimizer],
        schedulers: Sequence[Optional[AbsScheduler]],
        train_iter_factory: AbsIterFactory,
        valid_iter_factory: AbsIterFactory,
        ...
        if distributed_option.distributed:
            if trainer_options.sharded_ddp:
                dp_model_s = fairscale.nn.data_parallel.ShardedDataParallel(
                    module=model_s, # TODO
                    sharded_optimizer=optimizers,
                )
            else:
                dp_model_s = torch.nn.parallel.DistributedDataParallel(
                    model_s,  # TODO
                    device_ids=(
                        # Perform multi-Process with multi-GPUs
                        [torch.cuda.current_device()]
            ...
            dp_model_s = torch.nn.parallel.DataParallel(
                model_s,  # TODO
                device_ids=list(range(distributed_option.ngpu)),
            )
        else:
    ...
    def train_one_epoch(
        ...
        for iiter, (utt_id, batch) in enumerate(
            ...
            if np.random.random()>0.0 : # TODO, change that with different datasets after !
                # generate PLs :
